...
class FEMRBatchProcessor:
    ...
    def convert_dataset_from_list(self, patient_list, tokens_per_batch: int, min_patients_per_batch: int = 4, num_proc: int = 1):
        """Convert a list of MEDS patients to batches.

        Arguments:
            patient_list: A list containing MEDS patients
            tokens_per_batch: The number of tokens allowed per batch
            min_patients_per_batch: The minimum number of patients per batch
            num_proc: The number of processers to use when converting

        Returns:
            A list of batches ready to be processed
        """
        # Create a simple dataset-like structure from the list
        # Each item needs to have 'patient_id' and 'events' keys
        simple_dataset = [{"patient_id": patient["patient_id"], "events": patient["events"]} for patient in patient_list]
        print('test')
        print(len(simple_dataset))
        max_length = tokens_per_batch // min_patients_per_batch
        
        # Process all patients to get preliminary batch statistics
        lengths = []
        for patient_index, patient in enumerate(simple_dataset):
            batch = {"patient_id": [patient["patient_id"]], "events": [patient["events"]]}
            indices = [patient_index]
            lengths.extend(map_preliminary_batch_stats(batch, indices, processor=self, max_length=max_length))
        ...
        # Shuffle the lengths for better batch distribution
        rng = np.random.default_rng()
        rng.shuffle(lengths)
        ...
        for start, end in zip(batch_offsets[:-1], batch_offsets[1:]):
            self.creator.start_batch()
            for patient_index, offset, length in lengths[start:end, :]:
                self.creator.add_patient(patient_list[patient_index.item()], offset, length)
            
            result = self.creator.get_batch_data()
            if "task" in result:  # Only include batches with task data
                batches.append(self.creator.cleanup_batch(result))
        
        return batches


    def convert_patient_with_synthetic_labels(self, patient, offset=0, max_length=None, tensor_type=None, **formatter_kwargs):
        """
        Modified version of convert_patient that adds synthetic labels if none exist.
        This ensures that patients without labels still get processed.
        """
        self.creator.start_batch()
        self.creator.add_patient(patient, offset=offset, max_length=max_length)
        batch_data = self.creator.get_batch_data()
        
        # Check if there are no labels and add synthetic ones if needed
        if batch_data["transformer"]["label_indices"].shape[0] == 0:
            # Create a synthetic label at the last position in the sequence
            timeline_length = batch_data["transformer"]["valid_tokens"].shape[0]
            if timeline_length > 0:
                # Add a label at the last position
                batch_data["transformer"]["label_indices"] = np.array([timeline_length - 1], dtype=np.int32)
                
                # Add task data if a task exists
                if self.creator.task is not None and "task" not in batch_data:
                    # Create basic task data - this may need customization based on the task
                    batch_data["task"] = self.creator.task.get_batch_data()
                    batch_data["needs_exact"] = self.creator.task.needs_exact()
    ...
    def convert_dataset_from_list_with_synthetic_labels(self, patient_list, tokens_per_batch: int, min_patients_per_batch: int = 4, num_proc: int = 1):
        """
        Convert a list of MEDS patients to batches, adding synthetic labels where needed.
        """
        # Create a simple dataset-like structure from the list
        simple_dataset = [{"patient_id": patient["patient_id"], "events": patient["events"]} for patient in patient_list]
        ...
        # First, convert each patient and ensure they have labels
        preprocessed_patients = []
        for patient in patient_list:
            patient_data = self.convert_patient_with_synthetic_labels(patient, max_length=max_length)
            preprocessed_patients.append(patient_data)
        
        # Now extract the label indices to create batches
        lengths = []
        for patient_index, patient_data in enumerate(preprocessed_patients):
            if patient_data["transformer"]["label_indices"].shape[0] > 0:
                # Determine where to start and how long the sequence should be
                label_indices = patient_data["transformer"]["label_indices"]
                
                if hasattr(self.creator.task, 'needs_exact') and self.creator.task.needs_exact():
                    current_start = 0
                    current_end = 0
        ...
        for start, end in zip(batch_offsets[:-1], batch_offsets[1:]):
            ...
            if self.creator.task is not None and "task" not in result:
                result["task"] = self.creator.task.get_batch_data()
                result["needs_exact"] = self.creator.task.needs_exact()

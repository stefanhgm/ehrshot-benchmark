...
# Create grouped legends by model type (dict order = legend order)
    model_types = {'LLM': [], 'BERT': [], 'CLMBR': [], 'Other': []}
    for label in labels:
        if any(llm in label.lower() for llm in ['llm', 'gpt', 'qwen', 'qwen3']):
            ...
            model_types['CLMBR'].append(label)
        else:
...
def plot_one_labeling_function(df: pd.DataFrame,
    ...
    if labeling_function == 'new_celiac':
        ...
        df = df[df['k'] <= 64]
    elif labeling_function == 'Cardiac arrest':
        df = df[df['k'] <= 32]
    elif labeling_function == 'Abdominal aortic aneurysm':
        df = df[df['k'] <= 24]
    elif labeling_function == 'Aortic stenosis':
        df = df[df['k'] <= 48]
    elif labeling_function == 'Mitral valve insufficiency':
        df = df[df['k'] <= 64]
    elif labeling_function == 'Endocarditis':
        df = df[df['k'] <= 24]
    elif labeling_function == 'Rheumatic fever and chronic rheumatic heart diseases':
        df = df[df['k'] <= 64]
    elif labeling_function == "Parkinson's disease (Primary)":
        df = df[df['k'] <= 32]
    elif labeling_function == 'Suicide ideation and attempt or self harm':
        df = df[df['k'] <= 64]
    ...
    model_order = ['clmbr', 'qwen3', 'bioclinicalbert', 'count']
    ...
    # else:
    #     ax.set_title(labeling_function, pad=10, fontweight='bold')
    if change_legend:
        title = LABELING_FUNCTION_2_PAPER_NAME[labeling_function]
    else:
        title = labeling_function
    title_wrapped = textwrap.fill(title, width=20)  # adjust width as needed
    ax.set_title(title_wrapped, pad=10, fontweight='bold')
    ...
    ax.set_xlabel("# of Train Examples per Class", fontsize=6, fontweight='bold')
...
def plot_one_task_group(df: pd.DataFrame,
    ...
    if(task_group not in ["mean_all_task_groups", "UKB", "EHRSHOT"]):
        if(task_group == "Assignment of New Diagnoses" and not addlinesdiagnoses):
        ...
        if(task_group == "mean_all_task_groups"):
            task_groups = ['Mortality Prediction', 'Operational Outcomes', 'Assignment of New Diagnoses']
            combined_dfs = pd.DataFrame()
            for task in task_groups:
                df_filtered = filter_df(df, score=score, task_group=task, model_heads=model_heads)
            ...
            if(task_group == "UKB"):
                task_groups = ['OMOP_4306655', 'OMOP_9201', "disease_onset"]
            else:
                task_groups = ['chexpert', 'lab_values', 'operational_outcomes', 'new_diagnoses']
            combined_dfs = pd.DataFrame()
            for task in task_groups:
                df_filtered = df #filter_df(df, score=score, task_group=task, model_heads=model_heads)
                combined_dfs = pd.concat([combined_dfs, df_filtered], axis=0)
        
            cols_to_mean = ["value"]
            df_combined = (
                combined_dfs.groupby(
                    ["sub_task", "model", "head", "k", "score"],
                    as_index=False
                )[cols_to_mean]
                .mean()
            )
    ...
    if not task_group in ["UKB", "EHRSHOT"]:  # your boolean
        agg_dict.update({
            'lower': 'mean',
            'upper': 'mean',
        })
        ...
        for h_idx, head in enumerate(heads):
            ...
            for subtask in df_means_['labeling_function'].unique():
                ...
                if((shadedregion) and (subtask in ["death", "hospitalization", "disease_onset"]) and (not (addlinesdiagnoses and task_group == "Assignment of New Diagnoses")) and (not task_group == "mean_all_task_groups")):  # only add shaded regions for the 3 diagnosis subtasks in the Assignment of New Diagnoses task group, and not in the mean_all_task_groups which combines across task groups
                    ...
                    ax.plot(df_m_['k'], df_m_['value'], color=color, linestyle='-', linewidth=1, alpha=0.15) # faded lines
...
def plot_one_task_group_box_plot(df: pd.DataFrame,
    ...
    for model in models:
        ...
        for head in heads:
            ...
            values = []
            for v in df_[df_['k'] != full_data_k]['value']:
                v = np.array(v)
                
                if len(full_data_values) == 0:
                    continue

                min_len = min(len(v), len(full_data_values))
                values.append(v[:min_len] - full_data_values[:min_len])
            ...
            bp = ax.boxplot(
                values,
                positions=positions + shift_amt * width - width / 2 + (shift_amt * 0.05),
                widths=width,
                showfliers=False,
                manage_ticks=False,
                patch_artist=True
